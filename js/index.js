...
let fechaLimite = new Date(catalogo[0].fechaFinDescuento).getTime();/* busca la fecha que figura en el primer objeto de mi catalogo */
...
let intervalo = setInterval(function(){
    ...
    let ahora = new Date().getTime(); /* feche y hora actual */
    let distancia = fechaLimite - ahora;

    let dias = Math.floor(distancia / (1000 * 60 * 60 * 24)); /* los dias que faltan */
    let horas = Math.floor((distancia % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));/* las horas que faltan */
    let minutos = Math.floor((distancia % (1000 * 60 * 60)) / (1000 * 60)); /* los minutos que faltan */
    let segundos = Math.floor((distancia % (1000 * 60)) / 1000); /* los segundos que faltan */
 
    dias = dias < 10 ? '0' + dias : dias; /* agrega 0 adelante de los dias menores a 10 */ /* condición ? valor si es verdadero : valor si es falso */
    ...
    if(distancia < 0){ /* si la cuenta regresiva ya termino*/
        clearInterval(intervalo); /* deja de repetir la funcion */

        if(document.querySelector('#conteoDias')){ /* cuando termina deja los numeros en 00 para que no haya negativos */
...
}, 1000); /* para que se actualice todos los segundos */
...
function cargarProductosDestacados() {
    contenedorDestacados.innerHTML = " "; /* limpia */ 

    let productosRandom = [];

    while (productosRandom.length < 3) { /* while para que no repita productos */
        let numeroRandom = Math.floor(Math.random() * catalogo.length);
        let productoRandom = catalogo[numeroRandom];

        if (productosRandom.includes(productoRandom) === false) { /* chequea si esta repetido el producto */
            productosRandom.push(productoRandom); /* si no estaba agregado lo pone */
        }
    }
